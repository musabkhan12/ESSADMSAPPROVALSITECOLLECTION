...
// import { getSP } from '../../essaApproval/loc/pnpjsConfig';
...
//     // Fetch Logos
//     await _context.web.lists.getByTitle("UtilitySettings").items().then((res: any) => {
...
//  this is for apfx2 and UAT site
    ...
    if (component === "DashboardProd") {
      await _context.web.lists.getByTitle("ARGSidebarNavigation").items.select("Title,Url,Icon,ParentId,ID,EnableAudienceTargeting,Audience/Title , IsActive").expand("Audience").orderBy("Order0", true)().then((res: any) => {
        console.log('%c res', "background-color:red", res);
        const items: NavItem[] = res.map((item: any) => {
          return {
            Title: item.Title,
      ...
      await _context.web.lists.getByTitle("ARGSidebarNavigation").items.select("Title,Url,Icon,ParentId,ID,EnableAudienceTargeting,Audience/Title , IsActive").expand("Audience").filter("IsActive eq 1").orderBy("Order0", true)().then((res: any) => {
        console.log('%c res', "background-color:red", res);
        const items: NavItem[] = res.map((item: any) => {
          return {
            Title: item.Title,
